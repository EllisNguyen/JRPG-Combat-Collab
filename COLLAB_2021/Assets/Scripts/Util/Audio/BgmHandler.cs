...
public class BgmHandler : MonoBehaviour
{
    ...
    [SerializeField] float defaultFadeSpeed = 1.8f;
    ...
    public void PlayOverlay(AudioClip bgm, int loopStartSamples, float fadeTime, bool canLoop = true)
    {
        //if overlay track is already playing the bgm, do not continue
        if (overlayTrack.clip != bgm || currentTrack != Track.Overlay)
        {
            StartCoroutine(PlayOverlayIE(bgm, loopStartSamples, fadeTime, canLoop));
        }
    }

    private IEnumerator PlayOverlayIE(AudioClip bgm, int loopStartSamples, float fadeTime, bool canLoop = true)
    {
        loop = canLoop;
